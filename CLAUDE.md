...
- **`PersistentNotification` lifetimes.** A `regular` notification's countdown starts at its *first showing* (`seen_at`: the panel open in a visible tab, or the toast of a Django message) — `success`/`info` 10 s, `warning`/`danger` 10 min (`NOTIFICATION_TTL_AFTER_SEEN`); one never shown is dropped after `PERSISTENT_NOTIFICATION_TTL_HOURS`. `export` and `release` go only by hand. `confirmation` (a supplier import awaiting a decision, `ref='import_run:<pk>'`) is deleted by `supplier_product_manager.tasks.dismiss_confirmations` wherever an `ImportRun` leaves `STATUS_NEEDS_CONFIRMATION` — call it on any new such transition. Read through `.visible()`.

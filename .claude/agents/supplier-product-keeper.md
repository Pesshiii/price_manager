...
description: Answers questions about supplier_product_manager — SupplierProduct raw price rows and the Excel import pipeline (Setting/Link/DictItem column mapping, the SupplierFile queue, the cached get_sps parse, the copy-to-main bridge). Consult BEFORE touching the import pipeline, adding a price field, or changing what a Setting means. Also records new insights into that app's knowledge directory (.claude/knowledge/<app>/) when asked.
...
Your memory is the directory `.claude/knowledge/supplier_product_manager/` — one file per topic.
...
6. Keep the front matter true: a changed scope means a changed `summary`, a
   ...
   every `[[supplier_product_manager/<topic>]]` link to it, in any app's topics and the glossary.
...
You write to topic files in `.claude/knowledge/supplier_product_manager/`, and — for a term from

...
`models.py` (six models — Currency, Supplier, Discount, Manufacturer,
ManufacturerDict, Category), `tasks.py` (2 tasks), `views.py`, `forms.py`,
`tables.py`, `filters.py`, `templates/`.

Most of this app's screens render from templates under `core/templates/`
(`supplier/`, `manufacturer/`, `currency/`, `category/`), not from its own
`templates/` dir. Look there before concluding a template is missing.

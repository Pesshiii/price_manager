...
Your memory is the directory `.claude/knowledge/retiring_stack/` — one file per topic.
...
6. Keep the front matter true: a changed scope means a changed `summary`, a
   ...
   every `[[retiring_stack/<topic>]]` link to it, in any app's topics and the glossary.
...
You write to topic files in `.claude/knowledge/retiring_stack/`, and — for a term from

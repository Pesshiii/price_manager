...
Your memory is the directory `.claude/knowledge/main_product_manager/` — one file per topic.
...
6. Keep the front matter true: a changed scope means a changed `summary`, a
   ...
   every `[[main_product_manager/<topic>]]` link to it, in any app's topics and the glossary.
...
You write to topic files in `.claude/knowledge/main_product_manager/`, and — for a term from

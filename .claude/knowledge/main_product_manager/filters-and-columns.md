...
confirmed gone from `urls.py`/`views.py`, no such route or view remains. Rows
are MainProduct, but search/brand/categories go **through
`MainProduct.product`** via [[product]]'s shared `matching_product_pks`.
Must not touch `MainProduct.search_vector`/`.categories`/`.manufacturer`
(removed from the model, confirmed absent from `filters.py`); `core/views.py`
imports it at module scope (`core/views.py:33`) — a stale field reference
stops the whole app booting. Rows without a Product are still found by own
`sku`/`name`/`article` (in the cart, invisible would mean unbuyable).
...
(`filters.py:195-196`) sets it via `product.filters.expanded_category_pks`
...
is built (`config_filters` runs at `filters.py:87`) — a Filter's `.field` is
...
(confirmed gone repo-wide: no `columns.py` in the app directory);
`/mainproduct/` permanently redirects to `/products/` (`urls.py:13`, no
query params carried over). This app's own `urls.py` still owns the per-row
routes: create/update/info/detail/logs and the pricetag-list proxy
(`urls.py:15-23`). This app's `tables.py` is tiny now too — `MainProductLogTable`
only (`tables.py:13-49`). The card's «Наценки» column (`product_price_manager`
`PriceTagList`) splits PriceTags into «Фиксированные» (`p_manager IS NULL`,
editable in place) and «Из менеджеров наценок» (opens the rule's modal — a
rule's apply overwrites its tags, so they are not edited here) — part of the
PR #231 rework of the detail page, along with the separate price-card layout
and the «Остаток» card in `detail.html` (see
[[main_product_manager/detail-page-and-photos]]).

## Three columns that look like fields but aren't

`MainProductLog` has three writers: `utils.update_logs()` (`utils.py:853-887`

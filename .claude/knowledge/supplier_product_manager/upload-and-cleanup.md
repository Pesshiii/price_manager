...
code: price_manager/supplier_product_manager/tasks.py, price_manager/supplier_product_manager/views.py
---
# Upload and cleanup

## Upload and cleanup — the file a setting depends on

- **`UploadSupplierFile.form_valid`** (`views.py:161`) reads the workbook's
  ...
  (`views.py:61`) and catches only `IntegrityError` inside a savepoint. Empty
  `Setting`s with a `(N)` suffix, no file and no links are likely leftovers of
  the old loop.
- **`cleanup_supplier_files_task`** (`tasks.py:384`) never deletes a setting's
  newest file: `keep_last` is clamped to at least 1 and
  `SUPPLIER_FILES_KEEP_LAST` defaults to 1 (`settings/celery.py:26`). It used to
  ...
  `STATUS_QUEUED`/`STATUS_RUNNING`/`STATUS_NEEDS_CONFIRMATION` are skipped too.
- **The upload view does not delete older files.** It used to, immediately —
  ...
  `ERROR` (`views.py:203-208`, they will never be imported), and the cleanup
  deletes them. As a second line, `_finish_run` (`tasks.py:65`) and the file
  log/status helpers write with queryset `update()`s, so a row deleted
  mid-import no longer fails the task.

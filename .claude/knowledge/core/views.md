...
## Views (`core/views.py`, ~816 lines)

The shopping-tab / cart feature is the whole file. `ShoppingTab*` — list, delete,
detail, export, export-download, import + preview + run (`:219`–`:515`).
`CartItem*` — detail, add-set, quick-add, confirm, unconfirm, remove,
product-select, add-products (`:518`–`:812`). Plus `PersistentNotification*`
(`:66`, `:88`), auth views (`:104`, `:124`), Bitrix24 login (`:133`, `:151`,
see [[core/bitrix24-login]]), and `mainpage` (`:815`). The user guide is not a
page here any more — it is release 0.0 in `releases` (data migration `0002`).
...
skill. `_get_shopping_tab_items` (`:284`) and `_shopping_tab_summary` (`:293`)
...
`supplier_manager/models.py:104`).
...
the table moved to `product/tables.py:426-433`
...
(`supplier_manager/models.py:141-154`) now has an explicit `if stock is None`

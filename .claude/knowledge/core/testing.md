...
`ExecuteLockedTaskAtomicTests` (`core/tests.py:36`) must stay a
...
runs and surfaces as `Currency.DoesNotExist` errors from
...
Decimal("1")})`, as [[supplier_product_manager]]'s `tests.py` and
`product_price_manager/tests.py:15` do. The inline `get_or_create(name='KZT',
value=1)` still at `main_product_manager/tests.py:14`/`:104`/`:251`/`:472` is
fragile: inline kwargs are *lookups*, so a KZT row carrying any other value
makes it attempt an INSERT and die on the unique `name`. `Currency` is the
only static seed in the tree.

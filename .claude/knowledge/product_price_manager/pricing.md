...
## `get_fitting_mps()` (`models.py:140`) — one supplier row per product, by construction

Returns the matching `MainProduct` queryset **annotated with `changed_price`**,
the post-markup value, computed via a `Subquery` (the `_changed_price`
annotation, built at `:220`/`:235`/`:250` depending on the source branch)
over `calc_qs`.
...
The docstring (`models.py:142`–`:147`) now records the real behaviour; any
other doc or comment promising a minimum predates #142.

The SP_PRICES branch (`:199`–`:215`) takes the **latest row by `updated_at`**:
...
(`self.mp.supplierproducts.order_by('-updated_at').first()`, `models.py:429`)
...
- **`clear_unsourced_prices()`** (`models.py:502`) runs last in
...
- `_clearing_candidates()` (`models.py:482`) prefilters with plain joins
...
reason, documented in a comment right above the field, `models.py:20`-`:23`).

Everything downstream — `save`, `apply`, `delete`, `deprecate` — calls this.
`get_price_querry` (`models.py:149`) has a commented-out earlier version
directly above the live one (`:150`–`:157`); don't mistake the dead block for
the implementation.

## `update_prices()` (`models.py:558`)

The bulk entry point, wrapped by `product_price_manager.update_prices`
(`tasks.py:9`-`:15`). Its inner `get_updated_mps(pricetags)` (`:559`) **merges
...
`clear_unsourced_prices()` runs once at the very end (`models.py:608`), after

...
def _dataframe_to_payload(df: pd.DataFrame, row_limit: int) -> dict:
    """Сериализует pandas DataFrame в JSON-friendly формат."""
    total = int(df.shape[0])
    head = df.head(row_limit)
    cleaned = head.where(pd.notna(head), None)
    rows = []
...
class PreviewView(APIView):
    ...
    def post(self, request):
        ...
        try:
            df = apply_partial(df_obj, file_obj, up_to=up_to)
        ...
        payload = _dataframe_to_payload(df, row_limit=data['row_limit'])

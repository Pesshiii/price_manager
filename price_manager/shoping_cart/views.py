...
from shoping_cart.forms import AlternateProductForm, ShopingTabCreateForm, ShopingTabUpdateForm
...
class ShoppingTabDetailView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['products'] = self.object.products.select_related('main_product').order_by('name')
        return context
...
class ShoppingTabProductCreateView(LoginRequiredMixin, View):
    ...
    def get_context(self, form):
        return {'form': form, 'tab': self.tab, 'is_update': False}
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            alternate_product, _ = AlternateProduct.objects.get_or_create(name=name, main_product=main_product)
...
class ShoppingTabSelectionView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        tabs = (
            ...
            .annotate(product_count=Count('products', distinct=True))
            .prefetch_related(
                Prefetch('products', queryset=AlternateProduct.objects.select_related('main_product').order_by('name'))
            )
            .order_by('name')
        )
        ...
        context.update({'product': product, 'tabs': tabs, 'existing_tab_ids': existing_tab_ids})
        return context
...
class ShoppingTabAddProductView(LoginRequiredMixin, View):
    ...
    def post(self, request, tab_pk, product_id):
        ...
        if alternate_product_id:
            alternate_product = get_object_or_404(AlternateProduct, pk=alternate_product_id, shoping_tabs=tab)
            already_linked = alternate_product.main_product_id == product.pk
            if not already_linked:
                (
                    tab.products.filter(main_product=product)
                ...
                status = 'success'
                message_text = f'Товар связан с «{alternate_product.name}» в корзине «{tab.name}».'
            else:
                status = 'info'
            ...
            if tab.products.filter(main_product=product).exists():
                ...
                status = 'success'
                message_text = f'Товар добавлен в корзину «{tab.name}».' if created else f'Товар привязан к корзине «{tab.name}».'
        context = {'tab': tab, 'product': product, 'status': status, 'message': message_text}

...
from supplier_manager.models import Category as SupplierCategory, Manufacturer, Supplier
...
class MainPageFilter(FilterSet):
  """Фильтр СТАРОЙ главной страницы — и только её.

  Держится ровно до Phase 2: он построен на search_vector, categories и
  manufacturer самого MainProduct, а Phase 2 удаляет их вместе со страницей.
  Всё, что переживает Phase 2 (корзина, «Привязать из ГП»), ходит через
  MainProductFilter выше. Новый код сюда не подключать.
  """
  class Meta:
    model = MainProduct
  ...
  def build_helper(self, url, hx_target: str | None = '#mainproducts-table', stripped=False):
    ...
    Only needed by a view that actually renders mainproduct/partials/filter.html
    (MainProductFilterView) — other callers only need `.qs`, so
  ...
  def config_filters(self, queryset):
    ...
    category_queryset = SupplierCategory.objects.filter(pk__in=queryset.values('categories')).get_ancestors(include_self=True)
    selected_categories = SupplierCategory.objects.filter(pk__in=self.data.getlist('categories', None))
    if selected_categories:
      category_queryset = SupplierCategory.objects.filter(Q(pk__in=category_queryset)|Q(pk__in=selected_categories))
  ...
  def categories_method(self, queryset, name, value):
    ...
    categories = SupplierCategory.objects.filter(query)

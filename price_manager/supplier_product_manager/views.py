...
class UploadSupplierFile(CreateView):
  ...
  def form_valid(self, form):
    ...
    if not instance.setting:
      ...
      while not created:
        try:
          setting = Setting.objects.create(name = os.path.splitext(instance.file.name)[0] + 'Дупликат'*number, supplier = Supplier.objects.get(pk=self.kwargs.get('pk')))
          created = True
        except:
          created = False
    else:
    ...
    if not os.path.splitext(instance.file.name)[1] == '.csv':
      ...
      setting.save()
    if not instance.setting:
      redirect_url = reverse('setting-detail', kwargs={'pk':setting.pk}) 
      return HttpResponseClientRedirect(redirect_url)
    for sfile in sfs:
      if sfile.name == setting.sheet_name:
...
class XMLTableView(TemplateView):
  ...
  def get_context_data(self, **kwargs) -> dict[str, Any]:
      context = super().get_context_data(**kwargs)
      file = None
      setting = Setting.objects.get(pk=self.kwargs.get('pk'))
      for sf in SupplierFile.objects.filter(setting=setting):
        if os.path.splitext(sf.file.name)[0] == setting.sheet_name:
          file=sf.file
      if not file:
        file = SupplierFile.objects.filter(setting=setting).first().file
      df = pd.read_csv(file).dropna(axis=0, how='all').dropna(axis=1,how='all').fillna('')
...
class SettingDetail(UpdateView):
  model = Setting
  form_class = SettingForm
  template_name = "supplier_product/setting_detail.html"
  def get_form(self):
    form = super().get_form(self.form_class)
    form.fields['sheet_name'].choices = [(os.path.splitext(sf.file.name)[0] , os.path.splitext(sf.file.name)[0]) for sf in SupplierFile.objects.filter(setting=self.kwargs.get('pk'))]
    return form
  def get_context_data(self, **kwargs) -> dict[str, Any]:
      context = super().get_context_data(**kwargs)
      file = None
      for sf in SupplierFile.objects.filter(setting=self.kwargs.get('pk')):
        if os.path.splitext(sf.file.name)[0] == self.get_object().sheet_name:
          file=sf.file
      if not file:
        file = SupplierFile.objects.filter(setting=self.kwargs.get('pk')).first().file
      self.df = pd.read_csv(file)
      chs = []
      for i in range(len(self.df.columns)):
        ctx = {}
        ctx.update(csrf(self.request))
        form_html = render_crispy_form(LinkForm(self.request.POST if self.request.POST else None,prefix=f'link-{i}'), context=ctx)
        chs.append((self.df.columns[i], form_html))
      context["column_headers"] = chs
      return context

...
связи есть amount. Своих поставщиков у набора нет, поэтому существующий
конвейер его не видит: reindex_pim_ids толкает в PIM только Product с
MainProduct, а бэкфилл контента берёт только Product с pim_id. Отсюда
отдельная синхронизация, которая делает всё сама: находит или заводит Product
набора, пишет на него контент PIM и полностью пересобирает его состав.
...
def sync_product_sets() -> dict:
    ...
    for set_id, links in links_by_set.items():
        ...
        with transaction.atomic():
            ...
            if not product.name:
                # display_name у набора откатываться не на что: ни поставщиков,
                # ни PMP. Без названия строка показала бы голый артикул.
